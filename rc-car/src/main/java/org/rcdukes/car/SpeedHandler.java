...
class SpeedHandler {
  ...
  void handleMotor(JsonObject messageBody) {
    ...
    if ("up".equals(speed)) {
      ...
      if (currentSpeed > engineMap.neutral()
          && currentSpeed < engineMap.minForward()) {
        currentSpeed = engineMap.minForward();
      } else if (currentSpeed > engineMap.minReverse()
      ...
      if (currentSpeed < engineMap.neutral()
        ...
        currentSpeed = engineMap.minReverse();
      } else if (currentSpeed < engineMap.minForward()
          && currentSpeed > engineMap.neutral()) {
        currentSpeed = engineMap.neutral();
      }
  ...
  void handleSpeedDirect(JsonObject messageBody) {
    ...
    if (speedPercentage == 0) {
      ...
      double diff = ((Math.abs(engineMap.maxForward() - engineMap.minForward())
          * Math.abs(speedPercentage)) / 100);
      // LOG.debug("speedPercentage > 0, diff = " + diff);
      targetSpeed = engineMap.minForward() + diff;
    } else {
      double diff = ((Math.abs(engineMap.maxReverse() - engineMap.minReverse())
          * Math.abs(speedPercentage)) / 100);
      // LOG.debug("speedPercentage < 0, diff = " + diff);
      targetSpeed = engineMap.minReverse() - diff;
    }

    LOG.debug("set speed percentage to " + speedPercentage
        + ", target speed to " + targetSpeed);
  ...
  private void setMotorSpeedWrapper(int speed) {
    ...
    EngineMap engineMap = car.getEngine().getEngineMap();
    if (!engineMap.turnedOrientation()) {
      // reverse speed because motor was initially connected the wrong way
      // around
