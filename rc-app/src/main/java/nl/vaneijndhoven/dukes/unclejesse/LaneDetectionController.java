...
public class LaneDetectionController {
   ...
   *          - the original image
   * @param cannyConfig
   *          - the CannyEdgeDetector configuration
   * @param lineDetectorConfig
   *          - the lineDetector configuration
   * @param collector
   *          - an imageCollector
   */
  public void performLaneDetection(Mat originalImage,
    ...
    ImageLaneDetection laneDetect = null;
    if (laneDetect == null)
      throw new RuntimeException("Can't run - need to be fixed");
    // @FIXME
    // new ImageLaneDetection(cannyConfig, lineDetectorConfig);
    ...
    if (currentTime - tsLastCommand > COMMAND_LOOP_INTERVAL) {
      ...
      if ((previousAngle != 0)
        ...
        if (rudderPercentage > 0) {
          ...
          // System.out.println("steering " + direction + ", percentage: " +
          // rudderPercentage);
